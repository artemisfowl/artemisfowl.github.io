...
`__getattr__` and the other is the `__getattribute__` magic function. When we create an instance of a class, or as we call an object, Python basically calls `getattr(instance, attribute_name)`.
